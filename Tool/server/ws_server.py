        ...
        while True:
            ...
            if 'LOC' in data and 'rapport_score' in data and 'utterances_data' in data:
                ...
                if session:
                    ...
                    # add code to wait if 2nd user's interval data is not present
                ...
                # fetch all-interval data of both users.
                print("End session block")
                session = db.sessions.find_one({"session_id": session_id})
                if session:
                    all_user_data = session.get("users", [])
                    response = {
                        "status": "success",
                        "message": "EndSession",
                        "all_user_data": all_user_data
                    }
                    #send it to client.py along with message "Endsession"
                    for conn_id, client in connected_clients.items():
                        # Skip the client that initiated the "EndSession"
                        if conn_id == connection_id:
                            continue
                        websocket1 = connected_clients[conn_id]
                    await websocket1.send_text(json.dumps(response))
                    await websocket1.close()
                #wait to recieve finalStats
                #send finalStats to extension.ts and close server.
                pass
